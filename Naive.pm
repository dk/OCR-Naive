# $Id: Naive.pm,v 1.14 2009/08/05 14:16:06 dk Exp $
package OCR::Naive;
...
sub enhance_image
{
	...
	require Prima::IPA;
	require Prima::IPA::Misc;
	require Prima::IPA::Point;
	...
	my @h = (0, Prima::IPA::Misc::histogram( $i), 0);
	my @peaks =
		map { $_ - 1 }
		sort { $h[$b] <=> $h[$a] } 
	...
	warn "fg=$fg bg=$bg threshold=$threshold\n"
		if $options{verbose};
	$i = Prima::IPA::Point::threshold( $i, minvalue => $threshold);
